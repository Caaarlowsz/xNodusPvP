...
import Nodus.uvu.*;
import Nodus.Bloquinhos.*;
import Nodus.Managers.*;
import Nodus.InventariosFake.*;
import Nodus.Event.*;
import Nodus.Kits.*;
import Nodus.FuturaMLG.*;
import Nodus.Warp.*;
...
public class Main extends JavaPlugin implements Listener
{
    ...
    public static void removeAbility(final Player p) {
        ...
        Main.invencivel.remove(p.getName());
        ...
        GladiatorHAB.blocks.remove(p);
        GladiatorHAB.bloco.remove(p);
        GladiatorHAB.fighting.remove(p);
        GladiatorHAB.gladgladiator.remove(p);
        GladiatorHAB.localizacao.remove(p);
        GladiatorHAB.oldl.remove(p);
        GladiatorHAB.players.remove(p);
        GladiatorHAB.tasks.remove(p);
        JellyfishHab.naoescorrer.remove(p);
        ...
        InfernorHab.Infer.remove(p.getName());
    ...
    public void onPlugin4(final PlayerCommandPreprocessEvent event) {
        ...
        if (!p.hasPermission("meu.plugins") && (event.getMessage().toLowerCase().startsWith("/pl") || event.getMessage().toLowerCase().startsWith("/plugins") || event.getMessage().toLowerCase().startsWith("/?") || event.getMessage().toLowerCase().startsWith("/rl") || event.getMessage().toLowerCase().startsWith("/about") || event.getMessage().toLowerCase().startsWith("/achievement") || event.getMessage().toLowerCase().startsWith("/bukkit") || event.getMessage().toLowerCase().startsWith("/ver") || event.getMessage().toLowerCase().startsWith("/bukkit:") || event.getMessage().toLowerCase().startsWith("/bukkit:me") || event.getMessage().toLowerCase().startsWith("/help") || event.getMessage().toLowerCase().startsWith("/me") || event.getMessage().toLowerCase().startsWith("/reload") || event.getMessage().toLowerCase().startsWith("/bukkit:plugins"))) {
    ...
    public void onPlugin42(final PlayerCommandPreprocessEvent event) {
        final Player p = event.getPlayer();
        if (Main.usandokit.contains(p) && (event.getMessage().toLowerCase().startsWith("/1v1") || event.getMessage().toLowerCase().startsWith("/plugins") || event.getMessage().toLowerCase().startsWith("/?") || event.getMessage().toLowerCase().startsWith("/rl") || event.getMessage().toLowerCase().startsWith("/about") || event.getMessage().toLowerCase().startsWith("/achievement") || event.getMessage().toLowerCase().startsWith("/bukkit") || event.getMessage().toLowerCase().startsWith("/ver") || event.getMessage().toLowerCase().startsWith("/bukkit:") || event.getMessage().toLowerCase().startsWith("/bukkit:me") || event.getMessage().toLowerCase().startsWith("/help") || event.getMessage().toLowerCase().startsWith("/me") || event.getMessage().toLowerCase().startsWith("/reload") || event.getMessage().toLowerCase().startsWith("/bukkit:plugins"))) {
            event.setCancelled(true);
        }
    }
    ...
    public static void setScoreBoard(final Player p) {
        ...
        final Score nomeserver = o.getScore(Bukkit.getOfflinePlayer("§7" + ChatColor.translateAlternateColorCodes('&', Main.plugin.getConfig().getString("NomeServer"))));
        ...
        final Score a4 = o.getScore(Bukkit.getOfflinePlayer("§3          "));
        final Score a5 = o.getScore(Bukkit.getOfflinePlayer("§l   "));
        final Score a6 = o.getScore(Bukkit.getOfflinePlayer("§o    "));
        final Score a7 = o.getScore(Bukkit.getOfflinePlayer("§o      "));
    ...
    public void economias() {
        ...
        this.getConfig().options().copyDefaults(true);
        this.saveConfig();
        final RegisteredServiceProvider<Economy> economyProvider2 = (RegisteredServiceProvider<Economy>)this.getServer().getServicesManager().getRegistration((Class)Economy.class);
    ...
    public void colocaragua(final PlayerInteractEvent event) {
        final Player p = event.getPlayer();
        if (Main.mlg.contains(p.getName()) && p.getItemInHand().getType() == Material.GOLD_INGOT && event.getAction() == Action.RIGHT_CLICK_BLOCK) {
