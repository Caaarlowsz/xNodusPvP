...
public class Ghoul implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("ghoul")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.ghoul")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Ghoul.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Ghoul.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void interact(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Main.ghoul.contains(p.getName()) && p.getItemInHand().getType() == Material.ROTTEN_FLESH) {
