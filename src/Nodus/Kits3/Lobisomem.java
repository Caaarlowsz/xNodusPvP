			...
			if (!Main.usandokit.contains(p.getName())) {
				if (p.hasPermission("kit.lobisomem")) {
					...
					final World w = Bukkit.getServer()
							.getWorld(Lobisomem.plugin.getConfig().getString("adafafadfafafafd.world"));
					...
					p.sendMessage(ChatColor.translateAlternateColorCodes('&',
							Lobisomem.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
				}
			} else {
				p.sendMessage(ChatColor.translateAlternateColorCodes('&',
						Lobisomem.plugin.getConfig().getString("Um_Kit_Por_Vida")));
			}
			return true;
		}
		return false;
