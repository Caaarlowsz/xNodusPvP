...
public class Sponger implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("sponger")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.sponger")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Sponger.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Sponger.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void lancar(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Main.sponger.contains(p.getName()) && p.getItemInHand().getType() == Material.SPONGE) {
            e.setCancelled(true);
            p.updateInventory();
            if (!Sponger.cooldown.containsKey(p.getName()) || Sponger.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                if (Main.SemPvP(p)) {
                ...
                p.sendMessage(ChatColor.RED + "Faltam " + TimeUnit.MILLISECONDS.toSeconds(Sponger.cooldown.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar novamente.");
            }
        }
    }
    ...
    public void teia(final ProjectileHitEvent e) {
        if (e.getEntity() instanceof Egg && e.getEntity().getShooter() instanceof Player) {
            ...
            if (Sponger.teias.containsKey(p.getName()) && teia == Sponger.teias.get(p.getName())) {
                ...
                Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Sponger.plugin, (Runnable)new Runnable() {
                    @Override
                    public void run() {
            ...
            for (final Entity ent : p.getNearbyEntities(4.0, 4.0, 4.0)) {
                if (ent instanceof Player) {
                    ...
                    if (plr.isSneaking()) {
                        ...
                        plr.sendMessage(ChatColor.GRAY + "Voce Foi pisoteado Pelo sponger: " + ChatColor.AQUA + p.getName());
                    }
                    else {
