...
public class Hulk implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("hulk")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.hulk")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Hulk.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Hulk.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void pegar(final PlayerInteractEntityEvent e) {
        ...
        if (e.getRightClicked() instanceof Player) {
            ...
            if (Main.hulk.contains(p.getName()) && (!Hulk.cooldown.containsKey(p.getName()) || Hulk.cooldown.get(p.getName()) <= System.currentTimeMillis()) && p.getItemInHand().getType() == Material.AIR) {
                e.setCancelled(true);
                p.updateInventory();
