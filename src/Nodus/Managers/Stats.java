...
public class Stats implements Listener
{
    ...
    public void a(final Player player, final Player killer) {
        if (player.getKiller() instanceof Player) {
            try {
                this.a.getConfig().set("jogador." + killer.getName().toLowerCase() + ".kill-streak", (Object)(this.a.getConfig().getDouble("jogador." + killer.getName().toLowerCase() + ".kill-streak") + 1.0));
                if (this.a.getConfig().getInt("jogador." + killer.getName().toLowerCase() + ".kill-streak") >= this.a.getConfig().getDouble("jogador." + killer.getName().toLowerCase() + ".max-kill-streak")) {
                    this.a.getConfig().set("jogador." + killer.getName().toLowerCase() + ".max-kill-streak", (Object)this.a.getConfig().getDouble("jogador." + killer.getName().toLowerCase() + ".kill-streak"));
                }
            }
            catch (Exception e) {
            ...
            try {
                if (this.a.getConfig().getDouble("jogador." + player.getName().toLowerCase() + ".kill-streak") >= this.a.getConfig().getInt("jogador." + player.getName().toLowerCase() + ".max-kill-streak")) {
                    this.a.getConfig().set("jogador." + player.getName().toLowerCase() + ".max-kill-streak", (Object)this.a.getConfig().getInt("jogador." + player.getName().toLowerCase() + ".kill-streak"));
                }
            }
            catch (Exception e) {
    ...
    public void a(final PlayerDeathEvent event) {
        ...
        if (event.getEntity().getKiller() instanceof Player) {
            ...
            if (i >= 5 && i % 5 == 0) {
                Bukkit.getServer().broadcastMessage(ChatColor.translateAlternateColorCodes('&', this.a.getConfig().getString("messages.killstreak-5").replace("{NUM}", Integer.toString(i)).replace("{PLAYER}", killer.getName())));
            }
        }
        else {
            this.a.getConfig().set("jogador." + player.getName().toLowerCase() + ".kill-streak", (Object)0);
