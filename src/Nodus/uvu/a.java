...
public class a implements Listener {
	...
	public static Location getLocation(final String s) {
		...
		return new Location(Bukkit.getWorld(loc[0]), (double) Double.valueOf(loc[1]), (double) Double.valueOf(loc[2]),
				(double) Double.valueOf(loc[3]), (float) Float.valueOf(loc[4]), (float) Float.valueOf(loc[5]));
	}
	...
	public void setLocation(final String s, final Location l) {
		final String location = String.valueOf(String.valueOf(l.getWorld().getName())) + ":" + String.valueOf(l.getX())
	...
	public void onItemDrop(final PlayerDropItemEvent event) {
		if (a.jogadores.contains(event.getPlayer())) {
		...
		if (e.getEntity() instanceof Player) {
			if (e.getDamager() instanceof Player) {
				if (a.jogadores.contains(e.getEntity())) {
					...
					if (!a.lutadores.containsKey(((Player) e.getEntity()).getName()) && !a.lutadores.containsValue(p.getName())) {
						e.setCancelled(true);
					}
				}
				...
				if (Nodus.uvu.a.jogadores.contains(target)) {
					...
					} else if (Nodus.uvu.a.lutadores.containsKey(target.getName())
							&& Nodus.uvu.a.lutadores.get(target.getName()).equalsIgnoreCase(p2.getName())) {
						return;
					}
					e.setCancelled(true);
				}
	...
	public void Quit(final PlayerQuitEvent e) {
		...
		if (a.lutadores.containsKey(e.getPlayer().getName())) {
			final Player matou = Bukkit.getServer().getPlayerExact((String) a.lutadores.get(e.getPlayer().getName()));
			...
			for (final Map.Entry<String, String> m : a.lutadores.entrySet()) {
	...
	public void Death(final PlayerDeathEvent e) {
		...
		if (a.lutadores.containsKey(e.getEntity().getPlayer().getName())) {
			final Player matou = Bukkit.getServer()
					.getPlayerExact((String) a.lutadores.get(e.getEntity().getPlayer().getName()));
			final Player morreu = e.getEntity().getPlayer();
			final Damageable hp1 = (Damageable) matou;
			matou.sendMessage(" §7§l[1v1]  §3Voce matou §7" + morreu.getName() + " §3que ficou com §7"
					+ String.valueOf(this.GetSoupAmount((Inventory) morreu.getInventory())) + " §3sopas restantes.");
			morreu.sendMessage(" §7§l[1v1]  §3Voce morreu para §7" + matou.getName() + " §3que ficou com§7 "
					+ (int) hp1.getHealth() / 2.0 + " §3coracoes e §7"
					+ String.valueOf(this.GetSoupAmount((Inventory) matou.getInventory())) + " §3sopas restantes.");
			Set1v1.daritens(matou);
	...
	public void InteractItem(final PlayerInteractEvent e) {
		if (a.jogadores.contains(e.getPlayer())) {
			if ((e.getAction().equals((Object) Action.RIGHT_CLICK_AIR)
					...
					&& e.getItem().getType() == Material.REDSTONE) {
				e.setCancelled(true);
				final Player p = e.getPlayer();
			...
			if ((e.getAction().equals((Object) Action.RIGHT_CLICK_AIR)
					|| e.getAction().equals((Object) Action.RIGHT_CLICK_BLOCK))
					&& e.getItem().getType() == Material.SLIME_BALL) {
				e.setCancelled(true);
				final Player p = e.getPlayer();
				if (!a.randomQueue.contains(p)) {
					a.randomQueue.add(p);
			...
			if ((e.getAction().equals((Object) Action.RIGHT_CLICK_AIR)
					|| e.getAction().equals((Object) Action.RIGHT_CLICK_BLOCK))
					&& e.getItem().getType() == Material.THIN_GLASS) {
				e.setCancelled(true);
	...
	public void InteractItem(final PlayerInteractEntityEvent e) {
		if (a.jogadores.contains(e.getPlayer()) && e.getRightClicked() instanceof Player
	...
	public void interactmenu(final InventoryClickEvent e) {
		if (e.getWhoClicked() instanceof Player && a.jogadores.contains(e.getWhoClicked())
