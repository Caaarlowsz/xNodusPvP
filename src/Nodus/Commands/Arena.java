...
public class Arena implements CommandExecutor, Listener {
	...
	public boolean onCommand56789(final CommandSender sender, final Command cmd, final String commandLabel,
			final String[] args) {
		final Player p = (Player) sender;
	...
	public boolean onCommand09876(final CommandSender sender, final Command cmd, final String commandLabel,
			final String[] args) {
		...
		if (commandLabel.equalsIgnoreCase("check") && p.hasPermission("nodus.check") && args.length == 1) {
			...
			if (target != null) {
				if (!target.isOp()) {
					final int killst = Main.plugin.getConfig()
							.getInt("jogador." + p.getName().toLowerCase() + ".kill-streak");
