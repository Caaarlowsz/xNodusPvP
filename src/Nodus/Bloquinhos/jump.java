...
public class jump implements Listener
{
    ...
    public void onPlayerJum2p(final PlayerMoveEvent e) {
        ...
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.CLAY) {
            ...
            Bukkit.broadcastMessage("§6O jogador: §7" + e.getPlayer().getName() + " §3Ganhou 500$ por passar o nivel Hard do challenge!");
            p.closeInventory();
    ...
    public void onPlayerJwum2p(final PlayerMoveEvent e) {
        ...
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.HARD_CLAY) {
            Main.economy.depositPlayer(e.getPlayer().getName(), 500.0);
            Bukkit.broadcastMessage("§6O jogador: §7" + e.getPlayer().getName() + " §3Ganhou 1000$ por passar o Parkour!");
            p.closeInventory();
            p.getInventory().clear();
            p.sendMessage("");
            p.sendMessage("");
    ...
    public void onPlayerMove(final PlayerMoveEvent event) {
        ...
        if (Main.mlg.contains(player.getName()) && (player.getLocation().getBlock().getType() == Material.WATER || player.getLocation().getBlock().getType() == Material.STATIONARY_WATER)) {
            Main.mlgss(player);
    ...
    public void onPlayerFrente(final PlayerMoveEvent e) {
        ...
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.DIAMOND_BLOCK) {
            ...
            p.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 100, 999));
            p.getPlayer().playEffect(p.getPlayer().getEyeLocation(), Effect.STEP_SOUND, (Object)Material.DIAMOND_BLOCK);
            p.getWorld().playSound(p.getLocation(), Sound.FIREWORK_LAUNCH, 2.0f, 2.0f);
            p.setVelocity(sponge);
        }
    }
    ...
    public void onPlayerCamel(final PlayerMoveEvent e) {
        final Player p = e.getPlayer();
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.ICE || e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.PACKED_ICE) {
    ...
    public void onPlayerCamel2(final PlayerMoveEvent e) {
        final Player p = e.getPlayer();
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.SOUL_SAND || e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.SOUL_SAND) {
