        ...
        if (label.equalsIgnoreCase("Switcher")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.Switcher")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Switcher.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Switcher.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
