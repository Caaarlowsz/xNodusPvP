        ...
        if (label.equalsIgnoreCase("boxer")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.boxer")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Bomber.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Bomber.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
