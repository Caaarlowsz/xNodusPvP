...
public class Endermage implements Listener, CommandExecutor
{
    ...
    public void EndermageInteract(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && mage.getItemInHand().getType() == Material.PORTAL && Main.endermage.contains(mage.getName())) {
            e.setCancelled(true);
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if (!target.getInventory().contains(Material.PORTAL) && Endermage.this.isEnderable(bLoc, target.getLocation()) && target != mage && !target.isDead()) {
                            ...
                            if (!mage.getInventory().contains(new ItemStack(Material.PORTAL)) && Main.endermage.contains(mage.getName())) {
        ...
        if (label.equalsIgnoreCase("endermage")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.endermage")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Endermage.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Endermage.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
