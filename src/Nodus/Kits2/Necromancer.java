...
public class Necromancer implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("necromancer")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.Necromancer")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Necromancer.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Necromancer.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void interact(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Main.Necromancer.contains(p.getName()) && p.getItemInHand().getType() == Material.OBSIDIAN) {
            if (!Necromancer.cooldown.containsKey(p.getName()) || Necromancer.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                e.setCancelled(true);
                p.updateInventory();
            ...
            p.sendMessage(ChatColor.RED + "Faltam " + TimeUnit.MILLISECONDS.toSeconds(Necromancer.cooldown.get(p.getName()) - System.currentTimeMillis()) + " segundos para poder usar novamente.");
        }
    }
}
