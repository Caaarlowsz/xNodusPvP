...
public class Phantom implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("phantom")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.phantom")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Phantom.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Phantom.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void voar(final PlayerInteractEvent e) {
        ...
        if (e.getAction().name().contains("RIGHT") && p.getItemInHand().getType() == Material.FEATHER) {
            e.setCancelled(true);
            p.updateInventory();
            if (Main.phantom.contains(p.getName()) && (!Phantom.cooldown1.containsKey(p.getName()) || Phantom.cooldown1.get(p.getName()) <= System.currentTimeMillis())) {
