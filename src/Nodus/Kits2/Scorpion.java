...
public class Scorpion implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("scorpion")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.Scorpion")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Scorpion.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Scorpion.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void lancar(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Main.scorpionarrow.contains(p.getName()) && p.getItemInHand().getType() == Material.ARROW) {
            e.setCancelled(true);
            p.updateInventory();
            if (!Scorpion.cooldown.containsKey(p.getName()) || Scorpion.cooldown.get(p.getName()) <= System.currentTimeMillis()) {
                if (Main.SemPvP(p)) {
