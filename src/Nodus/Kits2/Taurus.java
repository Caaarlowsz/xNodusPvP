...
public class Taurus implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("taurus")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.taurus")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Taurus.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Taurus.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void SonicLick(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().getItemInHand().getType() == Material.EMERALD_BLOCK && Main.Sonic.contains(event.getPlayer().getName())) {
