...
public class Sonic implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("sonic")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.Sonic")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Nodus.Kits2.Sonic.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Nodus.Kits2.Sonic.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void SonicLick(final PlayerInteractEvent event) {
        ...
        if (event.getPlayer().getItemInHand().getType() == Material.LAPIS_BLOCK && Main.Sonic.contains(event.getPlayer().getName())) {
