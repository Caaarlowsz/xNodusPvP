...
public class Rider implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("Rider")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.Rider")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Rider.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Rider.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void onClick888(final InventoryClickEvent event) {
        if (event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.CARROT_ITEM && event.getWhoClicked().getVehicle() != null && this.horses.containsValue(event.getWhoClicked().getVehicle())) {
    ...
    public void onEntityDeath(final EntityDeathEvent event) {
        if (this.horses.containsValue(event.getEntity())) {
            ...
            final Iterator itel = this.horses.keySet().iterator();
            while (itel.hasNext()) {
