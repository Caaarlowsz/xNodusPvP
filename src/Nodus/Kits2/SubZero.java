...
public class SubZero implements Listener, CommandExecutor {
			...
			if (!Main.usandokit.contains(p.getName())) {
				if (p.hasPermission("kit.SubZero")) {
					...
					p.sendMessage(ChatColor.translateAlternateColorCodes('&',
							SubZero.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
				}
			} else {
				p.sendMessage(ChatColor.translateAlternateColorCodes('&',
						SubZero.plugin.getConfig().getString("Um_Kit_Por_Vida")));
			}
			return true;
		}
		return false;
	...
	public void BloodClick(final PlayerInteractEvent event) {
		...
		if (event.getPlayer().getItemInHand().getType() == Material.ICE
				&& Main.SubZeroarrow.contains(event.getPlayer().getName())) {
			...
			if (damager instanceof Snowball) {
				...
				if (snowball.getShooter() instanceof Player) {
					...
					Bukkit.getScheduler().scheduleAsyncDelayedTask(Main.plugin, (Runnable) new Runnable() {
						@Override
						public void run() {
							hit.getLocation().getWorld().playEffect(hitloc, Effect.STEP_SOUND, (Object) Material.ICE,
									20);
