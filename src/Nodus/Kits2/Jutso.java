...
public class Jutso implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("Jutso")) {
            if (!Main.usandokit.contains(p.getName())) {
                if (p.hasPermission("kit.jutso")) {
                    ...
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Jutso.plugin.getConfig().getString("Sem_Permiss\u00c3£o_Kit")));
                }
            }
            else {
                p.sendMessage(ChatColor.translateAlternateColorCodes('&', Jutso.plugin.getConfig().getString("Um_Kit_Por_Vida")));
            }
            return true;
        }
        return false;
    ...
    public void Clicar(final PlayerInteractEvent e) {
        ...
        if (p instanceof Player && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Main.jutso.contains(p.getName()) && p.getItemInHand().getType() == Material.PAPER) {
            e.setCancelled(true);
