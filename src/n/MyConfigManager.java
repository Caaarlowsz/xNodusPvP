...
public class MyConfigManager
{
    ...
    private File getConfigFile(final String file) {
        ...
        File configFile = this.getConfigFile(file);
        if (file.contains("/")) {
            if (file.startsWith("/")) {
                final File configFile2 = new File(this.plugin.getDataFolder() + file.replace("/", File.separator));
            }
            else {
                final File file2 = new File(this.plugin.getDataFolder() + File.separator + file.replace("/", File.separator));
            }
        }
        else {
    ...
    public void setHeader(final File file, final String[] header) {
        ...
        try {
            ...
            while ((currentLine = reader.readLine()) != null) {
                final String currentLine2 = currentLine;
                config.append(String.valueOf(currentLine2) + "\n");
            }
            reader.close();
            config.append("# +----------------------------------------------------+ #\n");
        ...
        try {
            ...
            while ((currentLine = reader.readLine()) != null) {
                ...
                if (currentLine2.startsWith("#")) {
                    final String addLine = currentLine2.replaceFirst("#", String.valueOf(pluginName) + "_COMMENT_" + commentNum + ":");
                    whole.append(String.valueOf(addLine) + "\n");
                    ++commentNum;
                }
                else {
                    whole.append(String.valueOf(currentLine2) + "\n");
                }
            }
        ...
        for (int length = (array = lines).length, i = 0; i < length; ++i) {
            ...
            if (line.startsWith(String.valueOf(this.getPluginName()) + "_COMMENT")) {
                ...
                if (comment.startsWith("# +-")) {
                    ...
                    if (lastLine == 0) {
                        config.append(String.valueOf(normalComment1) + "\n");
                    }
                    else if (lastLine == 1) {
                        config.append("\n" + normalComment1 + "\n");
                    }
