...
import Nodus.Managers.MyConfig;
...
public class MyConfigManager {
	...
	private File getConfigFile(final String file) {
		...
		if (file.contains("/")) {
			if (file.startsWith("/")) {
				new File(this.plugin.getDataFolder() + file.replace("/", File.separator));
			} else {
				new File(this.plugin.getDataFolder() + File.separator + file.replace("/", File.separator));
			}
		} else {
		...
		try {
			...
			while ((currentLine = reader.readLine()) != null) {
				...
				if (currentLine2.startsWith("#")) {
					final String addLine = currentLine2.replaceFirst("#",
